...
export class TablalogrosComponent implements OnInit {
  ...
  getinfo(){
    this.achievementservice.getassignedLogros_menu(this.data.Periodo,this.data.IDSubject).subscribe(res=>{
      this.logrosasigned=res;
    })
    this.achievementservice.getlogros_asignar(this.data.IDLevelGrade,this.data.IDSubject).subscribe(res=>{
      this.logros=res;
      this.comparar(res);
  ...
  comparar(arreglo:any){
    ...
    arreglo.forEach(logro => {
        this.logrosasigned.forEach(logroasignado => {
            if(logro.IDAchievement==logroasignado.IDAchievement.IDAchievement)
            { 
              logro.set=true;
            }
  ...
  Guardar(){
    ...
    this.logroobjeto.IDSubject=this.data.IDSubject;
  ...
  organizararreglo(){
    ...
    this.logrosasigned.forEach(logroviejo => {
      this.logros.forEach(logronuevo => {
          if(logroviejo.IDAchievement.IDAchievement==logronuevo.IDAchievement && logronuevo.set==false)
          { 
    ...
    this.logros.forEach(logronuevo => {
      incluye=false;
      this.logrosasigned.forEach(logroviejo => {
          if(logroviejo.IDAchievement.IDAchievement==logronuevo.IDAchievement)
          { 
            incluye=true;
          }
      });
